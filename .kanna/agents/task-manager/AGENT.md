...
Use `kanna_machine_stats` before creating a task that will build or test, and before advancing a stage whose next agent starts a full gate. It returns every reachable machine's 1/5/15-minute load averages, CPU core count, memory state, heavy build/test process counts, and busy-task count. Heavy processes are the closer build-lane signal; busy tasks preserve session-level context but are only a proxy because an agent may be reading while a child build runs. The snapshot reserves nothing and may already be stale. Hold to roughly two concurrent full gates per machine; past that the gates get slower rather than the work getting faster. A peer in `machineErrors` is unknown capacity, never idle capacity: assume it is loaded and stagger.
