...
A `HUMAN-REVIEW-DECISION` line means a person read that pull request at that exact commit and authorized this merge. Kanna recorded the decision durably before sending you anything, so the record survives the review session, the machine it was taken on, and the triage task that ranked it.
